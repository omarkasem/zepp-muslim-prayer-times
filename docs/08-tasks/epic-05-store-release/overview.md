...
- [x] Airplane mode after first location fetch: all screens render; reminders still fire (cached location).
- [x] `app.json` permissions exactly match real usage — remove any unused permission.
- [x] Confirm `appId: 1115692`, name "Muslim Prayer Times" / "مواقيت صلاة المسلم"; bump `version.code`/`name`.
- [x] No debug leftovers / console spam across all targets (Bip 6 + gt.r + gt.s).

## Step 2 — Store assets  (checklist)
- [x] Final app icon per target: 240×240 PNG, circular, transparent background, no padding.
...
- [x] `docs/app-store-listing.md` final: EN + AR name/intro/details, Features Descriptions, Privacy Policy
      text, permission selections, SDK=No, music=No.
- [x] Replace `[your contact email]` in the privacy policy; set "Last updated" to the submission date.
- [ ] If the store requires a privacy-policy **URL**, publish the policy (e.g. `PRIVACY.md` via GitHub Pages)
      and link it. (PRIVACY.md is ready in the repo root; not yet published to a public URL.)
